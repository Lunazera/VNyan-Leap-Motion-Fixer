...
namespace Leap_Motion_Fixer
{
    class Sja_Button : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            if (LZ_UI.settings.ContainsKey(buttonName))
            {
                // If it did exist set the button state to the loaded value.
                buttonState = Convert.ToSingle(LZ_UI.settings[buttonName]);
            ...
            else
            {
                ...
                LZ_UI.settings.Add(buttonName, Convert.ToString(buttonState));
